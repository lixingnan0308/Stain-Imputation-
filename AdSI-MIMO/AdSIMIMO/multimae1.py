...
class MultiMAE(nn.Module):
    ...
    def __init__(self,
        ...
        self.proj_mask = nn.Linear(dim_tokens, 
                                   self.output_adapters[self.output_domains[0]].dim_tokens)
        self.proj_vis = nn.Linear(dim_tokens, 
                                   self.output_adapters[self.output_domains[0]].dim_tokens)   
        self.layer_norm = nn.LayerNorm(self.output_adapters[self.output_domains[0]].dim_tokens)
    ...
    def generate_random_masks(self,
                            ...
                            alphas: Union[float, List[float]] = 2.0,
                            sample_tasks_uniformly: bool = False,
                            fixed_keys: list = ["dapi","af"],
                            masked_keys: list = ["cd68","cd16","sox10"]) :
        """
        ...
        alphas = [alphas] * len(masked_keys) if isinstance(alphas, float) else alphas
        if sample_tasks_uniformly:
            alphas = self.sample_alphas(B, len(masked_keys), alphas=alphas)
        ...
        task_masks = []
        num_tokens_per_task = [task_tokens.shape[1] for name, task_tokens in input_tokens.items()]
        num_masked_tokens = num_tokens_per_task[0] * len(masked_keys) - num_encoded_tokens  

        def distribute_tokens(num_tokens_per_task, task_sampling_dist, num_encoded_tokens):
            batch_size, num_tasks = task_sampling_dist.shape
            
            # Per-sample token allocation results
            tokens_per_task = torch.zeros_like(task_sampling_dist, dtype=torch.long)
            
            for j in range(batch_size):
                # Step 1: Initial per-sample token count from Dirichlet distribution
                masked_tokens_per_block = (task_sampling_dist[j] * num_masked_tokens).round().long()

                # Clamp each domain's masked count to at most 196 (one full patch grid)
                for i in range(len(masked_tokens_per_block)):
                    if masked_tokens_per_block[i] > num_tokens_per_task[0]:
                        excess = masked_tokens_per_block[i] - num_tokens_per_task[0]
                        masked_tokens_per_block[i] = num_tokens_per_task[0]
                        
                        # Redistribute excess tokens to other domains
                        for _ in range(excess):
                            idx = torch.randint(0, len(masked_tokens_per_block), (1,)).item()
                            while idx == i or masked_tokens_per_block[idx] >= num_tokens_per_task[0]:
                                idx = torch.randint(0, len(masked_tokens_per_block), (1,)).item()
                            masked_tokens_per_block[idx] += 1
                diff = num_masked_tokens - masked_tokens_per_block.sum()

                if diff != 0:
                    for _ in range(abs(diff)):
                        if diff > 0:
                            # Find domains below capacity and increment
                            candidates = torch.where(masked_tokens_per_block < num_tokens_per_task[0])[0]
                            if len(candidates) > 0:
                                idx = candidates[torch.randint(0, len(candidates), (1,)).item()]
                                masked_tokens_per_block[idx] += 1
                        else:
                            # Find domains above zero and decrement
                            candidates = torch.where(masked_tokens_per_block > 0)[0]
                            if len(candidates) > 0:
                                idx = candidates[torch.randint(0, len(candidates), (1,)).item()]
                                masked_tokens_per_block[idx] -= 1
                
                tokens_per_task[j] = num_tokens_per_task[0] - masked_tokens_per_block

            return tokens_per_task
        
        if num_encoded_tokens == 0:
            samples_per_task = torch.zeros(B, len(self.output_domains), device=device, dtype=torch.long)
        else:
            samples_per_task  = distribute_tokens(
                num_tokens_per_task, 
                task_sampling_dist, 
                num_encoded_tokens
                )
        ...
        for i, num_tokens in enumerate(num_tokens_per_task):
            ...
            biomarker_name = self.input_domains[i]
            if biomarker_name not in self.output_domains:
                ...
                mask = torch.where(mask < torch.tensor(fixed_num_tokens), 0, 1)
        ...
        ids_keep = ids_shuffle[:, :num_encoded_tokens+fixed_num_tokens*(len(self.input_domains)-len(self.output_domains))]
        ids_masked = ids_shuffle[:, num_encoded_tokens+fixed_num_tokens*(len(self.input_domains)-len(self.output_domains)):]


        # Update binary mask to adjust for task rounding
        mask_all = torch.ones_like(mask_all)
        mask_all[:, :num_encoded_tokens+fixed_num_tokens*(len(self.input_domains)-len(self.output_domains))] = 0
    ...
    def forward(self, 
        ...
        if task_masks is None:
            task_masks, ids_keep, ids_restore, ids_masked= self.generate_random_masks(
                input_task_tokens,
                num_encoded_tokens,
                alphas=alphas,
                sample_tasks_uniformly=sample_tasks_uniformly,
                masked_keys=self.output_domains
            )
        else:
        ...
        # Apply mask
        visual_tokens = torch.gather(input_tokens, 
                                    dim=1, 
                                    index=ids_keep.unsqueeze(-1).repeat(1, 
                                                                        1, 
                                                                        input_tokens.shape[2]))

        mask_tokens = torch.gather(input_tokens, 
                                    dim=1, 
                                    index=ids_masked.unsqueeze(-1).repeat(1, 
                                                                        1, 
                                                                        input_tokens.shape[2]))
        # Add global tokens to input tokens
        global_tokens = repeat(self.global_tokens, '() n d -> b n d', b=B)
        visual_tokens = torch.cat([visual_tokens, global_tokens],
                                   dim=1)
