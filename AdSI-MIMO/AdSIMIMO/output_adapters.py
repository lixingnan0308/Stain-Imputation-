...
class SpatialOutputAdapter(nn.Module):
    ...
    def __init__(self,
        ...
        if not self.learnable_pos_emb:
            self.pos_emb = build_2d_sincos_posemb(h=h_posemb, w=w_posemb, embed_dim=self.dim_tokens) #1，256，14，14
    ...
    def get_queries_and_context(self, context_tokens, input_info, ids_keep, ids_restore,mask_tokens):
    ...
    def forward(self,
        ...
        mask_tokens = torch.zeros_like(mask_tokens).to(mask_tokens.device)
        # Get queries and context
        queries, context_tokens = self.get_queries_and_context(encoder_tokens, 
                                            input_info, 
                                            ids_keep, 
                                            ids_restore,
                                            mask_tokens)
